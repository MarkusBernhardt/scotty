...
public class KeyValueGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all keyValues").get("/api/key-values").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new keyValue")
                        .post("/api/key-values")
                        .headers(headersHttpAuthenticated)
                        .body(StringBody("{" + "\"kvKey\": \"SAMPLE_TEXT\"" + ", \"kvValue\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_keyValue_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created keyValue").get("${new_keyValue_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created keyValue").delete("${new_keyValue_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the KeyValue entity").exec(scn);
